...
public class TracksEndpoint {
  ...
  public TracksEndpoint(final Video video, final ExtendedAnnotationService eas) {
    this.video = video;
    this.eas = eas;
  }
    ...
    return run(array(name), new Function0<Response>() {
      ...
      public Response apply() {
        ...
        try {
          ...
          final Track t = eas.createTrack(video.getId(), name, trimToNone(description), option(access),
        ...
        if (video.isSome()) {
          return eas.getTrack(id).fold(new Option.Match<Track, Response>() {
            ...
            public Response some(Track track) {
              ...
              final Track updated = new TrackImpl(id, video.get().getId(), name, trimToNone(description), trimToNone
                      (settings),
